...
soc: fw
	verilator --binary -j 0 -Wall $(RTL)/picorv32.v $(RTL)/top.v $(RTL)/axi_lite_interconnect.v $(RTL)/axi_decoder.v $(RTL)/rom.v $(RTL)/sram.v $(RTL)/uart_axi.v $(RTL)/uart_tx.v $(RTL)/uart_rx.v $(TB)/tb_top.v --top tb_top --timing --CFLAGS "-std=c++20" --trace
